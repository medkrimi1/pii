...
export class UserProfileSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.validationForm = this.formBuilder.group({
      ...
      about: [this.currentUser.about, [Validators.required, Validators.maxLength(300)]],
    }); 
  ...
  UpdateUser() {
    ...
    this.userService.checkEmailExists(email).subscribe((exists) => {
      if (exists && (this.currentUser.email !== this.form.email.value)) {
        ...
        this.currentUser.about = this.form.about.value;
        this.currentUser.dateNaissance = this.form.dateNaissance.value;
